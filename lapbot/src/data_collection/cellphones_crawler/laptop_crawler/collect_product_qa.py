...
OUTPUT_CSV_FILE = 'cellphones_items_comments.csv'
...
QUERY_TEMPLATE = """
    query COMMENTS ($currentPage: Int!, $pageUrl: String!, $productId: Int!){
          comment(
              type: "product", 
              pageUrl: $pageUrl,
              productId: $productId, 
...
def read_product_data(filename):
    """Read product_id and url_path from csv file"""
    product_data = []
    try:
        with open(filename, mode='r', encoding='utf-8-sig') as csvfile:
            reader = csv.DictReader(csvfile)
            product_id_col = 'product_id'
            url_path_col = 'url_path'
            if product_id_col not in reader.fieldnames or url_path_col not in reader.fieldnames:
                print(f'Error in finding {product_id_col} and {url_path_col} columns in csv file')
                print(f'Columns in file are: {reader.fieldnames}')
                return []

            for row in reader:
                try:
                    prod_id = int(row[product_id_col])
                    url_path = row[url_path_col]
                    if prod_id and url_path:
                        product_data.append({'id': prod_id, 'url_path': url_path})
                    else:
                        print(f'Error in missing product_id or url_path: {row}')
                except KeyError as e:
                    print(f'Skip row due to lack of key {e}: {row}')

            print(f'Successfully read {len(product_data)} products')
            return product_data
    except FileNotFoundError:
        print(f'Could not find {filename}')
        return []
    except Exception as e:
        print('Unwanted errors in reading file: ', e)
        return []


def fetch_all_comments(product_id, url_path):
    """Fetch all comments using pagination"""
    all_comments = []
    current_page = 1
    previous_matches_data = None
    page_url = BASE_URL + url_path.lstrip('/')

    print(f'Fetching comments from {page_url}')
...
def main():
    ...
    # Step 01: Read product_id and url_path from csv file
    products_to_scrape = read_product_data(INPUT_CSV_FILE)
    if not products_to_scrape:
        print('No products to process')
        return
    all_comments_raw = []
    # Step 02: Loop for each product and fetch comment
    for product in tqdm(products_to_scrape, desc='Fetching comments'):
        product_id = product['id']
        url_path = product['url_path']
        comments =  fetch_all_comments(product_id, url_path)
        if comments:
            all_comments_raw.extend(comments)
            time.sleep(1)
    print(f'Fetched {len(all_comments_raw)} comments')

    # Step 03: Process comment
    all_comments_processed = process_comment_data(all_comments_raw)
    # Step 04: Save to csv
    save_to_csv(all_comments_processed, OUTPUT_CSV_FILE)
